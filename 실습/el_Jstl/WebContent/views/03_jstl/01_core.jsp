...
<body>
	...
	* 변수 출력(c:out)
	...
	- 위의 태그 사용 시 조건식은 test라는 속성의 값으로 지정 해야 함
	...
	- 각 조건들은 c:choose문의 하위 요소로 c:when을 통해서 작성
	(default 값으로는 c:otherwise)
	</pre>
	
	<c:set var="no" value="2"/>
	<c:choose>
		<c:when test="${ no eq 0 }">
			처음 뵙겠습니다. <br>
		</c:when>
	...
	<h3>4. 반복문 - forEach 
	(c:forEach var="" begin="" end="" step="" items="" varStatus="")</h3>
	<pre>
	&lt;c:forEach&gt; 반복 시 실행할 내용 &lt;/c:forEach&gt;
	- Java의 for문에 해당하는 기능을 제공.
	- forEach문은 여러가지 속성이 사용 됨
	...
	- 해당 태그를 통해 url 경로와 관련 된 쿼리 스트링 값을 미리 설정하여 이를 제어할 수 있음
	</pre>
