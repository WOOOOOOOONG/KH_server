...
public class ElServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// JSP 내장 객체 범위
		// 1. page : 현재 해당 페이지에서만 사용 가능 -> 공유 범위가 가장 작음
		// 2. request : 요청받고 응답 페이지에서까지 사용 가능 -> 공유 범위가 다음으로 큼(하지만 제한적)
		// 3. session : 한 브라우저당 1개 존재하므로 모든 jsp에서 사용 가능 -> 공유 범위는 모든 jsp
		// 4. application :한 애플리케이션당 1개 존재하므로 애플리케이션 전역에서 사용 가능 -> 공유 범위가 가장 큼
		...
		// request, session, apllication 영역에 같은 속성명으로 값을 담기
		request.setAttribute("scope", "request영역");
		session.setAttribute("scope", "session영역");
		
		ServletContext application = request.getServletContext(); // ServletContext가 application인가 보다
		application.setAttribute("scope", "application영역");
		// 같은 attribute 이름으로 request, session, application이 있다면, 좁은 영역부터 우선순위를 가진다.
