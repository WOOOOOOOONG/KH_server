...
<%
	// 1. request, session 객체에 저장된 속성 scriptlet 이용하여 화면 출력
...
<body>
	...
	<!-- EL은 Scriptlet과 다르게 getter를 사용하지 않고 변수명을 통해 직접 접근 가능 
		내부적으로는 해당 객체의 getter를 자동적으로 활용하여 저장된 값을 읽어옴 -> ${ teacher.name }은 Person의 getName()을 읽어오는 것
	-->
	...
	<!-- 내부적으로 Person 클래스의 getter를 호출하고 있으므로 주석하면
		500 에러 발생하는것 확인할 수 있음! -->
		
	<hr>
	
	<h3>3. EL 사용 시 내장 객체에 저장된 속성명이 같은 경우</h3>	
	...
	<!-- 
		여러 스코프에 동일한 속성을 기록했다면, 이름 충돌(naming coplict)에 주의하여
		명시적으로 스코프를 지정해야 한다.
	 -->
</body>
</html>